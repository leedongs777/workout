...
- **세트 무게 입력 = −/+ 스텝퍼**(`wtCell`/`adjWeight`). 직접 타이핑도 되지만 폰트는 **반드시 16px 이상**이어야 iOS가 입력 포커스 시 화면을 자동 확대하지 않는다(과거 14px이라 확대 불편 있었음). 빈 칸에서 +를 누르면 `seedWeight`가 이전 세트(없으면 지난 기록) 무게를 먼저 채운다.
  ...
  - **기본값 프리필(`defaultWeight`, `.wt.pre` 흐리게)**: 저장된 기록이 없으면 시작 무게를 세트마다 미리 채운다. `카테고리 기본값 + (세트-1)×(setInc×기구단위)`. 편집하거나 +/− 누르면 `pre` 해제되며 저장.
  - ⚠️ **`WBASE`(카테고리 시작 무게)는 근거 없는 편의 추정치**다. 성별(여 ×0.65)·근력경험(초보 ×0.6)으로 스케일. `exCat`로 운동을 5개 카테고리(lower_big/lower_iso/push/pull/small)로 분류. 사용자가 설정 "무게 기본값"에서 카테고리별 기본값과 `setInc`(세트당 증량 배수 0~3)를 직접 조절(`state.wDefaults`/`state.setInc`).
