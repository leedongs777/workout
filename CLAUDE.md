...
## 1. 파일 구조 (단일 파일 + PWA 부속)
- 앱 전체가 **하나의 HTML 파일 `index.html`**에 들어 있음(임베드된 아이콘 마스크 때문에 ~500KB). 바닐라 JS(`<script>` 인라인) + `<style>` 인라인.
- **PWA 부속 파일(2026-07 추가)**: `manifest.webmanifest`, `sw.js`, `icons/`(192/512/maskable/apple-touch). 홈 화면 설치 + 오프라인용. ⚠️ **`deploy.yml`의 `_site` 복사 단계에 반드시 포함**되어야 한다(안 그러면 404라 설치 배너가 안 뜬다).
  - `sw.js`는 **HTML을 네트워크 우선**으로 처리한다. index.html이 앱 전체라 캐시 우선으로 바꾸면 배포해도 사용자가 옛 버전에 갇힌다. **되돌리지 말 것.**
  - Firebase/구글 도메인은 SW가 캐시하지 않는다(오래된 인증·동기화 응답 방지).
  - 앱을 크게 고쳐 캐시를 강제로 비우고 싶으면 `sw.js`의 `CACHE_VERSION`을 올린다.
  - `apple-touch-icon`은 **반드시 PNG**(iOS는 SVG 터치아이콘 미지원 — 과거 SVG data URI라 아이폰 홈 화면 아이콘이 깨졌음).
...
- **세트 수 ± 스텝퍼(`volEditor`→`chgSets`)**: `chgSets`에 넘기는 기준값은 **화면 표시값 `effVol().sets`(=`v.sets`)**여야 한다. `slot.sets`(raw)를 넘기면 목표(+1)·초보자(−1) 보정이 반영된 표시값과 어긋나 첫 탭이 값을 건너뛴다(2026-07 수정). 되돌리지 말 것.
  ...
  - 일반: `.setcell.wtc{flex:1 1 118px}`로 폭이 좁으면 자동 줄바꿈(폰에서 보통 2칸씩). **좌/우(측면당)은 표를 뒤집어 "한 줄 = 한 세트, 열 = 좌/우"**(행 라벨 `1세트…`, 헤더 `좌|우`, 2026-07 개편). 예전처럼 세트 헤더를 위에 몰고 좌/우를 행으로 두면 4세트에서 칸이 접혀 헤더와 어긋난다 — 되돌리지 말 것. 저장 키(`oi+'L'/'R'`, k)는 그대로라 레이아웃 변경이 기존 기록에 영향 없음.
...
- ⚠️ **한글 텍스트에 넓은 자간(letter-spacing) 금지** — `.15em` 같은 트래킹은 영문 대문자 마이크로라벨 전용 관습이고, 네모꼴 한글에선 글자가 벌어져 어색하다(운영자 제보로 2026-07 전면 정리: 한글 라벨은 `.01~.02em` 이하). 영문 전용 요소(`REST` pill, `Zone N` 배지)만 넓은 자간 허용.
...
- 우선순위: ①GitHub Actions 자동배포(**완료**), ②**PWA 설치·오프라인(완료, 2026-07 — 웹푸시 알림은 아직)**, ③localStorage→Firebase Firestore(확장 핵심), 이후 스크린샷 회귀 테스트·파일 경량화(아이콘 외부화)·결제·본인인증(NICE/포트원)·웹푸시 리마인더·React/Next 이관(성장 시).
