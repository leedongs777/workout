...
- 노출 여부는 `slotActive(sl)` 한 곳에서만 판단한다(`postureSym` 경유). `sessionExercises`·`excludedByInjury`·`compoundsInPlan` 모두 이걸 거친다.
...
- **웜업 순서**: ① 가벼운 유산소(체온) → ② 전신 부위별 단일 동작 순서 **목 → 어깨 → 팔 → 허리(상체 회전) → 옆구리 → 고관절(레그 스윙) → 허벅지(니 하이 마치) → 종아리(카프 레이즈) → 손목 → 발목** → ③ 그날 부위 활성화(상체: 밴드 풀 어파트 + [당기기날엔 턱 당기기(자세교정+거북목 목표일 때만, 위 참조) / 밀기=벽 팔굽혀펴기]; 하체: 바디웨이트 스쿼트 + 밴드 사이드 스텝; 전신·기타(`full`): 바디웨이트 스쿼트 + 밴드 풀 어파트 + 힙 서클; 유산소: 힙 서클).
