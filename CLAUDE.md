...
- ⚠️ **기본값은 중립적이어야 한다.** `workoutDays`는 **주 4일 중립값 `DEFAULT_WORKOUT_DAYS=[1,3,5,6]`(월·수·금·토)로 통일**했다(2026-07, 과거 주 6일은 특정 개인 일정이라 폐기 — 되돌리지 말 것). 아직 `DEFAULT_EQUIP`(27종)은 특정 헬스장 구성이라 재검토 대상(§7.5). 온보딩을 마친 사용자는 자기 값을 쓰므로 영향은 첫 진입·둘러보기 모드에 한정된다.
