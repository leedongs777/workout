...
- 5개 목표 전부 매핑 완료(유지=default):
  ...
  - `diet:[8,2,1,5,3,2,4,5]` 근력4:유산소4(1:1) — 대사 컨디셔닝 + 근육 보존.
  - `endur:[9,2,0,1,9,5,2,4]` 유산소 우위(5:3) — 인터벌 중심, 근력 보조.
  - `posture:[1,10,4,0,5,3]` 교정 강조 — 당기기/밀기/코어교정/집중일 + 하체 2.
...
- S6~S10은 특정 프로그램에서만 참조되므로 다른 목표 사용자에겐 안 나옴. 역할·장비 ID는 기존 것 재사용(신규 role 만들지 말 것 — ROLE_INFO/ROLE_PAT 키). 웜업/쿨다운은 title로 `sessionCat` 판정돼 자동 생성되므로 저작 불필요.
...
- ~~자세교정 판정이 세 군데(`GOALS`/`_POSTURE_ROLE`/`opt:'posture'`)에 흩어짐~~ → **해결됨(2026-07)**: role 정규식 폐기, 슬롯 데이터(`posture:true` / `opt:'posture:xxx'`)를 유일 소스로 통합. `postureSym`·`isPostureSlot` 헬퍼가 노출·세트+1 판정을 공유. 구정규식 대비 동작 동일(75슬롯 대조 0불일치) 검증(§4.4a).
