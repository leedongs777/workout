...
- **슬롯 데이터 표기(둘 중 하나로만 마킹)**:
  - **항상 노출 + 목표 시 세트 +1** → `posture:true`. 대상: `자세교정 (말린 어깨)`, `후면 삼각/능형근`, `자세교정 (하부 승모)`(S4). 근비대 가치가 함께 있어 항상 남긴다.
  - **증상 선택 시에만 노출** → `opt:'posture:neck|shoulder|back|pelvis'`. 대상: `거북목 교정`(neck), `자세교정 종합`(shoulder), `흉추 신전`·`자세교정 (하부 승모)`(S7)(back), `골반 정렬`(pelvis).
...
- **바닥 OFF 시 여전히 비는 슬롯 5개**(2026-07-13 실측): S0·S3 `골반 정렬`(글루트 브릿지 계열만), S3 `사이드 플랭크 (맥길)`, S4·S7 `자세교정 (하부 승모)`(프론 YTW만). 서서/기구 대안(스탠딩 골반 후방경사, 수트케이스 캐리, 밴드 YTW 계열 등) 보강 필요.
