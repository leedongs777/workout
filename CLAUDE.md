...
- **볼륨 밸런싱 패스(2026-07)**: 프로그램별 주간 볼륨을 실측(sessionExercises×effVol)해 과다·중복을 정리함.
  ...
  - **종목 다양화(2026-07)**: ① S9의 첫 슬롯을 전신(스텝)→메인 하체(고블릿 스쿼트)로 교체 — endur가 S9·S2를 각 2회 돌아 스텝업이 4일 나오던 것을 2일로. ② posture 순환을 [1,0,10,4,3,2]로 재배열 — 교정 무거운 S1·S10이 연달지 않게 사이에 하체A. ③ **S10의 옵션 순서를 S1과 반대로**(리어 델트 플라이·밴드 페이스풀 먼저) — 같은 주에 같은 기본 종목(리버스 펙덱·케이블 페이스풀)이 두 번 나오지 않게. 옵션 순서가 기본 선택을 정하므로(slotCands 안정 정렬) **의도적 순서 차이를 되돌리지 말 것**.
