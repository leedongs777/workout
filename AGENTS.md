# AGENTS.md — MATE (My AI Training Expert)

> AI 에이전트(Codex 등)용 프로젝트 온보딩 문서. **아래 본문은 `CLAUDE.md`와 동일하게 유지**한다(둘 중 하나를 고치면 다른 하나도 동기화).
> 언어: 앱 UI와 운영자와의 대화는 **모두 한국어**.
> ⚠️ 저장소는 공개일 수 있고 **커밋 히스토리에 실명·의료정보가 남아 있다**. 문서·코드·커밋에 **개인정보를 새로 적지 말 것**.
>
> ### 이 에이전트(Codex)의 주 작업 범위 — 아이콘 자산
> - **아이콘 생성/교체가 주 임무.** 아이콘 시스템은 아래 **§4.8** 참고(CSS mask + base64 임베드, 파일명=`EQ_LABEL` 1:1, 파이프라인: 투명PNG→알파 bbox crop→160px→팔레트+알파 64색→base64).
> - **아이콘 원본 PNG는 저장소 밖 프로젝트 루트**에 있다(저장소 기준 `../../`):
>   - `MATE_운동기구_아이콘_34종_투명배경/` (운동기구 34종, 파일명=EQ_LABEL 값과 1:1. 단 `/`가 든 라벨 2종은 파일명에서 `_`로 치환: `밴드_세라밴드.png`, `박스_플라이오박스.png`)
>   - `MATE_하단메뉴_아이콘_4종_투명배경/` (홈/운동/일정/설정 4종)
> - `index.html`의 `.ic-eq-{id}`/`.ic-pose-{key}`/`.ic-step-{key}` 마스크 클래스가 소비 지점. 동작 아이콘 27종은 2026-07 교체 완료(`MATE_동작_아이콘_27종_투명배경/`).
> - **다음 임무: 웜업·쿨다운 동작 애니메이션 38종** — 프롬프트·스펙·조립 가이드는 프로젝트 루트 `MATE_웜업쿨다운_애니메이션_생성프롬프트.md` 참고(키프레임 생성→WebP 루프 조립까지가 코덱스 몫).
> - ⚠️ **의료·안전·플랜 생성 로직(§3~§4.4)은 함부로 바꾸지 말 것.** 부득이 건드리면 반드시 **`node test/run.js`**(§2)로 회귀 검증 후 커밋.
---
